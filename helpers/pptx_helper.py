...
def generate_powerpoint_presentation(
    parsed_data: dict,
    ppt_demo_io: BytesIO | None
) -> Tuple[List, BytesIO]:
    """
    Create a PowerPoint presentation file in memory and return headers and blob.

    :param parsed_data: The presentation content as parsed JSON data.
    :param slides_template: The PPTX template to use.
    :return: A tuple containing:
        - A list of presentation title and slides headers.
        - A BytesIO object containing the generated PPTX file (blob).
    """

    # 初始化PPT对象并获取幻灯片尺寸（英寸）
    # presentation = pptx.Presentation(GlobalConfig.PPTX_TEMPLATE_FILES[slides_template]['file'])
    if ppt_demo_io:
        ppt_file = ppt_demo_io
    else:
        ppt_file = GlobalConfig.PPTX_TEMPLATE_FILES['Ouyeel-1']['file']
    ...
    for a_slide in parsed_data['slides']:
        try:
            ...
            print("is_processing_done 5")
        except Exception as exception:
            # 如果发生意外错误，尝试挽救当前内容
            logger.error(str(exception))
            continue

    presentation.core_properties.author = "Ouyeel"
    presentation.core_properties.comments = "Generated using Ouyeel AI"

    print(presentation.core_properties.author)
    print(presentation.core_properties.keywords)
    # 感谢页
    ...
    # 将PPTX文件写入内存中的BytesIO对象
    pptx_blob = BytesIO()
    presentation.save(pptx_blob)

    return all_headers, pptx_blob
...
def _handle_display_image__in_foreground(
    ...
    try:
        print("get_photo_url_from_api_response ing")
...
def _handle_icons_ideas(
    ...
    :return: True if the slide has been processed.
    """
    print("_handle_icons_ideas start")
    print(slide_json)
    icons_texts = None
    # if 'bullet_points' in slide_json and slide_json['bullet_points']:
    #     print("_handle_icons_ideas ing")
    ...
    #     # keyword = ' '.join([item[0] for item in icons_texts])
    #     # core_props.keywords = core_props.keywords + keyword
    #     print("_handle_icons_ideas find_icons ing")
    #     print(icons_texts)
    ...
    #         )
    #     print("_handle_icons_ideas find_icons True")
    #     return True
    print("_handle_icons_ideas find_icons False")
    return False
...
def download_pptx_to_io(url: str) -> BytesIO:
    response = requests.get(url)
    response.raise_for_status()  # 若下载失败将抛出异常
    return BytesIO(response.content)  # 内存中的二进制流
